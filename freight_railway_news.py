...
def fetch_and_summarize_news(client: anthropic.Anthropic) -> str:
    ...
    system_prompt = (
        ...
        "各記事について、必ず以下の3点を全て記載してください: "
        "①記事の要約（3〜5文）、②出典サイト名と記事タイトル、③記事の直接URL。"
        "URLは検索結果から必ず取得してください。URLが取得できない場合でも、"
        "サイトのトップページURLを代替として記載してください。絶対にURLを省略しないでください。"
    )

    user_prompt = f"""本日（{today}）の貨物鉄道に関するニュースを調査して、日本語でまとめてください。
...
■ 注目記事
各記事を以下のブロック形式で列挙してください。**要約・出典・URLは全て必須**です:

---
【記事タイトル】

▼ 要約
（この記事の内容を3〜5文で詳しく説明してください。何が起きたか、誰が関係しているか、影響は何か、を含めてください）

▼ 出典
- サイト名: （サイト名）
- 記事タイトル: （元の記事タイトル）
- URL: （記事の直接URL。不明な場合はサイトのトップページURL）
---
...
【重要な注意事項】
- 各記事のURLは必ず記載すること（省略・省エネ厳禁）
- URLが見つからない場合はサイトのトップページURLを記載すること
- 要約は必ず3文以上で記載すること
